...
async function loadUserInfo() {
  try {
    ...
    if (response.success && response.user) {
      elements.username.textContent = response.user.username || 'API User';
      elements.userEmail.textContent = response.user.email || 'Using API Key';
...
function createClipElement(clip) {
  ...
  div.addEventListener('click', () => {
    chrome.tabs.create({ url: `http://localhost:8002/clips/${clip.id}` });
  });
...
function setupEventListeners() {
  elements.connectBtn.addEventListener('click', async () => {
    // Just test the connection with current settings
    elements.connectBtn.disabled = true;
    elements.connectBtn.textContent = 'Testing connection...';
    ...
    try {
      ...
      if (response.success && response.connected) {
        ...
        alert('Connection failed. Please check your API settings.');
      }
    } catch (error) {
